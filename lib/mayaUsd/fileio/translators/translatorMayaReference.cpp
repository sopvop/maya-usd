...
static bool isSameFileName(const MString& found, const MString& expected)
{
    ...
    // when comparing the filesystem paths. Since we're only interested in
    // figuring if two references might point to the same file once resolved,
    // chop off the existing resolved reference to the length of the relative
    // file. Don't forget to remove the initial relative-path since the absolute
    // path won't contain them.
    const std::string expectedStr = TfStringTrimLeft(expectedPath.string(), "./\\");
    std::string       foundStr = foundPath.string();
    if (foundStr.size() < expectedStr.size())
        return false;
...
static MObject findExistingMayaReference(
    ...
    const MString& expectedUnresolvedFilePath)
{
    ...
    const bool hasExpectedRefName = (expectedRefName.length() > 0);
#ifdef MAYAUSD_DEBUG_MAYA_REFERENCE_LOCATION
    TF_WARN(
        "%s Maya custom ref attribute for %s",
        hasExpectedRefName ? "Has" : "No",
        prim.GetPath().GetText());
#endif
    if (!hasExpectedRefName) {
        return {};
    }
    ...
    for (MItDependencyNodes refIter(MFn::kReference); !refIter.isDone(); refIter.next()) {
        ...
        if (!isSameFileName(refFilePath, expectedUnresolvedFilePath)) {
#ifdef MAYAUSD_DEBUG_MAYA_REFERENCE_LOCATION
            TF_WARN(
                "Maya reference node [%s] does not refer to the expected file: "
                "expected [%s], found [%s].",
                expectedRefName.asChar(),
                expectedUnresolvedFilePath.asChar(),
                refFilePath.asChar());
#endif
            continue;
        }
...
MStatus UsdMayaTranslatorMayaReference::update(const UsdPrim& prim, MObject parent)
{
    ...
    if (refNode.isNull()) {
        refNode = findExistingMayaReference(prim, parentDag, mayaReferencePath, unresolvedPath);
    }
